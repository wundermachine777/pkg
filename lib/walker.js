...
import { ALIAS_AS_RELATIVE, ALIAS_AS_RESOLVABLE,
  ...
  isDotJS, isDotJSON, isDotNODE, isDotJSC, isPackageJson, normalizePath
    ...
    if (pkgConfig) {
      ...
      if (scripts) {
        ...
        for (const script of scripts) {
          ...
          if (stat.isFile()) {
            if (!isDotJS(script) && !isDotJSON(script) && !isDotNODE(script) && !isDotJSC(script)) {
